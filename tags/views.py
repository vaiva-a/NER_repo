...
def add_validator(request):
    if request.method == 'POST':
        try:
            ...
            # Check if username already exists in Validators table
            if Validators.objects.filter(username=username).exists():
                return JsonResponse({
                    'status': 'error',
                    'message': 'Username already exists in validators'
                })

            # Check if username already exists in Django User table
            ...
            # Generate next ID
            last_validator = Validators.objects.order_by('-ID').first()
            next_id = 1 if not last_validator else last_validator.ID + 1

            # Create Django user
            user = User.objects.create_user(username=username, password=password)
            
            # Create validator
            Validators.objects.create(
                ID=next_id,
                username=username, 
                password=password,
            )
...
def remove_validator(request):
    if request.method == 'POST':
        try:
            ...
            # First, delete from the Validators table
            validator = Validators.objects.filter(username=username).first()
            if validator:
                validator.delete()
            else:
                return JsonResponse({'status': 'error', 'message': 'Validator not found in Validators table.'})
            ...
            return JsonResponse({'status': 'success', 'message': f'Validator "{username}" removed successfully.'})
...
def auto_tag(request):
     if request.method == 'POST':
        ...
        domain = data.get("selectedDomain", "Gen")
        print(txttotag)
        url1=""
        ...
        if not task_type:
            ...
            if user and (user.is_staff or user.is_superuser):
                ...
                return render(request, "registration/login.html", {
                    "error": "Please select a task type to continue."
                })

        # For annotation tasks - check Annotators table
        if task_type == 'annotation':
            try:
                annotator = Annotators.objects.get(username=username, password=password)
                
                user = authenticate(request, username=username, password=password)

                if user is not None and annotator:
                    login(request, user)
                    request.session['task_type'] = task_type
                    request.session['annotator_id'] = annotator.ID
                    return redirect('/domain')
                else:
                    return render(request, "registration/login.html", {
                        "error": "Invalid credentials for annotation."
                    })
            except Annotators.DoesNotExist:
                return render(request, "registration/login.html", {
                    "error": "You are not authorized for annotation tasks."
                })
                
        elif task_type == 'validation':
            try:
                validator = Validators.objects.get(username=username, password=password)
                
                user = authenticate(request, username=username, password=password)
                
                if user is not None and validator:
                    login(request, user)
                    request.session['task_type'] = task_type
                    request.session['validator_id'] = validator.ID  
                    return redirect('/validation')
                else:
                    return render(request, "registration/login.html", {
                        "error": "Invalid credentials for validation."
                    })
            except Validators.DoesNotExist:
                return render(request, "registration/login.html", {
                    "error": "You are not authorized for validation tasks."
                })
...
def users(request):
    annotators = Annotators.objects.all()
    validators = Validators.objects.all()
    all_users = []
    
    for annotator in annotators:
        all_users.append({
            'username': annotator.username,
            'email': getattr(annotator, 'email', 'N/A'),
            'role': 'Annotator',
        })
    ...
    return render(request, 'tags/users.html', {'users': all_users})
...
def submit_validation(request):
    ...
    try:
        ...
        if is_valid:
            ...
            if domain == 'Gen':
                output_path = os.path.join(settings.BASE_DIR, 'output', 'final_gen.xlsx')
            elif domain == 'Med':
            ...
            return JsonResponse({
                'status': 'success',
                'message': f'Validation saved successfully'
            })
        else:
